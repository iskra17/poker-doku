...
- 흐름은 **Astra 작업 계획 → 필요한 경계만 Fable 검토 → Astra 확정 → Opus/Luna Max 구현·관련 테스트 →
  ...
  마이그레이션·지갑/원장·소켓 계약·권한/정답 누출·보상 reconcile·MTT 수명주기 변경은 독립 Fable 검토를 거친다.
  Astra가 직접 구현한 경우에도 이 경계 규칙은 동일하다.
...
- 동시 작업자는 총괄 외 **최대 2명**(CLI 에이전트 포함). 별도 worktree와 파일 소유권으로 분리하고,
  GPU 생성 워커는 1개만 실행한다. 전체 Vitest는 통합 배치당 `--maxWorkers=4`로 1회,
  개별 변경은 관련 검사만 수행한다. 전체 검사와 작업자 테스트는 겹치지 않게 한다.
