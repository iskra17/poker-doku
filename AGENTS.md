...
- 다음 확장: Ch7 라이브 리딩 퀴즈(`pendingQuiz`는 항상 null)·속마음의 공개 정책과 챕터 연결,
  Ch8~12 데이터, 하드 모드, 파트너별 Ch1 대사 변주, 2막 질문권(L4 힌트), 비비안/엘레나 의상.
  **가면 공개/행동 identity 분리는 이미 완료**됐으므로 퀴즈에서 기존 서버 seam을 활용한다.
  Ch1~6와 영상 49클립은 존재한다. 새 영상은 `scripts/art/story-video.md` 절차로 추가한다.
