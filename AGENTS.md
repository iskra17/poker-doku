...
## 개발 오케스트레이션 (2026-09-05 사용자 지정)

- **Astra X-High**가 전체 기획·작업 분해·의존성·리뷰 판정·통합의 최종 책임을 맡는다.
- **Fable 5.1**은 주요 기획 검토와 구현 후 심층 테스트를 맡는다. 특히 보상·상태 전환·재접속·
  권한/정답 누출·GPU 작업 복구를 독립 검증한다. 리뷰 의견은 재현 근거와 함께 총괄이 최종 판단한다.
- 구현은 **Claude Opus 또는 Astra Low/Medium**에 위임한다. Low는 승인된 대사·문구·에셋 매니페스트처럼
  범위가 좁은 작업, Medium/Opus는 UI 동작·서버 계약·보상·상태 머신·챕터 메커닉에 배정한다.
  objectives·프리셋 덱·스텝 스크립트는 단순 데이터가 아니므로 Low에 독자 설계를 맡기지 않는다.
...
- 작업 순서와 완료 조건: `docs/superpowers/specs/2026-09-05-development-orchestration-design.md`.
...
- 사용자 결정: 이번 세션은 Ch8·9와 아트 연결까지 종료한다. 다음 구현은 Ch10~12 실제 졸업 SnG이며
  `docs/superpowers/handoffs/2026-09-05-next-session-act4.md`부터 읽는다. Fable 검토 후 총괄 확정 설계는
