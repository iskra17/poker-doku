...
## 개발 오케스트레이션 (2026-09-06 사용자 변경 — 이전 역할 배정보다 우선)

- **Claude Fable 5.1**이 전체 기획·작업 분해·의존성·리뷰 판정·통합의 최종 책임을 맡는다.
- **GPT 6 Astra**는 기존 Fable 역할인 주요 기획 검토·코드 리뷰·구현 후 심층 테스트를 맡는다. 특히 보상·상태 전환·재접속·
  권한/정답 누출·GPU 작업 복구를 독립 검증한다. 리뷰 의견은 재현 근거와 함께 총괄이 최종 판단한다.
- 구현 서브에이전트는 **Claude Opus**에 위임한다. UI·서버 계약·보상·상태 머신·챕터 메커닉과 관련 테스트를
  담당하며, 작은 문구·에셋 등록도 같은 역할 배정을 따른다. Astra Low/Medium 구현 배정은 이전 결정이다.
- 흐름은 **Fable 작업 계획 → Astra의 필요한 경계 검토 → Fable 확정 → Opus 구현/관련 테스트 →
  Astra의 필요한 구현 검토 → Fable 최종 판단/통합**이다. 모든 작은 변경마다 양쪽 리뷰를 의무 반복하지 않는다.
...
- 동시 작업자는 총괄 외 **최대 2명**(CLI 에이전트 포함). 별도 worktree와 파일 소유권으로 분리하고,
  ...
  개별 변경은 관련 검사만 수행한다. 검증 후 커밋·main ff 병합, push/deploy는 사용자 지시 때만.
- 최신 시작점과 역할 계약: `docs/superpowers/handoffs/2026-09-06-fable-orchestration.md`.
  기존 작업 순서와 완료 조건: `docs/superpowers/specs/2026-09-05-development-orchestration-design.md`.
  과거 문서의 Astra 총괄/Fable 리뷰 배정은 이 사용자 변경으로 대체한다. 실제 세션 모델과 호출 모델을 확인하고,
