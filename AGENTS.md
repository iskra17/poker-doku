...
- **운영 백오피스** (`/admin` → `src/app/admin/page.tsx`, API는 `src/server/admin-http.ts`):
  `DEBUG_LOG_TOKEN`은 로그인 때만 검증하고 이후 요청은 불투명 HttpOnly `SameSite=Strict` 쿠키를 사용한다.
  기본 세션은 process-local 2시간이며, '이 기기에서 90일간 로그인 유지' 선택 시 SQLite
  `admin_trusted_devices`(v40)에 원문 대신 HMAC 조회 키를 저장해 재시작 후에도 유지한다.
  등록 세션은 GET session 성공 시 90일로 연장하며 로그아웃·등록 해제·운영 토큰 변경 시 무효화한다.
  변경 요청은 exact-origin + CSRF를 함께 검증하며,
...
## 개발 오케스트레이션 (2026-09-08 사용자 변경 — 이전 역할 배정보다 우선)

- **신규 사용자 QA**: `/admin`의 [새 사용자 QA] → `/qa`. 기존 관리자 세션·CSRF로 보호한
  `/api/admin/qa`가 원래 게임 자격을 HttpOnly 쿠키(`/api/admin` 경로)에 한 번 보관하고 게임 쿠키만
  전환한다. [다시 처음부터]는 원래 자격을 덮어쓰지 않으며 [원래 계정 복귀]로 복원한다. 계정 삭제·
  보상 우회·운영자 권한 부여는 없다. 쿠키는 브라우저 공용이므로 테스트 중 다른 게임 탭은 닫는다.
  QA iframe(`?qa=1`)의 소켓 토큰·첫 테이블 안내만 sessionStorage에 격리한다. QA 계정도 실제 프로필·
  플레이 로그로 남으므로 리텐션 분석 시 테스트한 프로필을 제외해야 한다. 회귀: `admin-qa-http.test.ts`.

- **비용·속도 우선 (같은 날 후속 지시)**: 작은 변경은 구현자 1명과 관련 검사로 끝낸다. 요청하지 않은
  관리 기능·설계 문서·다단계 리뷰를 추가하지 않는다. 추가 에이전트·독립 리뷰·전체 검사는 실제 영향 범위와
  복잡성이 필요로 할 때만 사용한다. 아래 역할 체계는 매 작업에 전원을 투입하라는 뜻이 아니다.
- **GPT 6 Astra**가 전체 기획·작업 분해·의존성·리뷰 판정·통합의 최종 책임을 맡는다.
- **Claude Fable 5.1**은 보조로 주요 기획 검토·코드 리뷰·구현 후 검토를 함께 맡는다. 특히 보상·상태 전환·재접속·
  권한/정답 누출·GPU 작업 복구를 독립 검증한다. 리뷰 의견은 재현 근거와 함께 총괄이 최종 판단한다.
- 구현 서브에이전트는 **Claude Opus**와 **GPT Luna Max**를 사용한다. Luna는 `gpt-5.6-luna` +
  `reasoning_effort: max`(모델명과 추론 노력은 별개)로 호출한다. Astra가 범위·난도·파일 소유권에 따라 배정한다.
  기본적으로 Opus는 서버 계약·보상·상태 머신 같은 복잡한 구현, Luna Max는 범위가 명확한 독립 구현·UI·관련 테스트를
  맡는다. 이 배정은 고정 기능 제한이 아니며, 모든 작은 수정마다 사용자에게 모델 선택을 다시 묻지 않는다.
- 흐름은 **Astra 작업 계획 → 필요한 경계만 Fable 검토 → Astra 확정 → Opus/Luna Max 구현·관련 테스트 →
  필요한 변경만 Fable 리뷰 → Astra 최종 판단·통합 검증**이다. 모든 작은 변경마다 양쪽 리뷰를 의무 반복하지 않는다.
  지갑/원장·소켓 계약·권한/정답 누출·보상 reconcile·MTT 수명주기처럼 영향이 큰 변경은 독립 Fable 검토를 활용한다.
  파일명이나 분야만으로 작은 수정에 리뷰 단계를 강제하지 않는다.
- 모델명과 추론 노력은 각각 기록한다. 실제 다른 모델의 결과를 Fable/Astra 결과로 부르지 않는다.
  Fable/Opus는 필요 시 로컬 Claude Code CLI로 호출하고 결과의 실제 모델 식별자를 확인한다.
- 동시 작업자는 총괄 외 **최대 2명**(CLI 에이전트 포함). 별도 worktree와 파일 소유권으로 분리하고,
  GPU 생성 워커는 1개만 실행한다. 개별 변경은 관련 검사만 수행한다. 전체 Vitest가 필요한 통합 배치는
  `--maxWorkers=2`로 1회 실행하고, 전체 검사와 작업자 테스트는 겹치지 않게 한다.
  junction worktree의 Turbopack 제약 때문에 build는 main 통합 후 수행하고, 실패하면 통합 완료로 보고하지 않는다.
  검증 후 커밋·main ff 병합, push/deploy는 사용자 지시 때만.
- 최신 시작점과 역할 계약: `docs/superpowers/handoffs/2026-09-08-astra-orchestration.md`.
  기존 작업 순서와 완료 조건: `docs/superpowers/specs/2026-09-05-development-orchestration-design.md`.
  2026-09-06의 Fable 총괄/Astra 리뷰 및 이전 Astra Low/Medium 구현 배정은 이 사용자 변경으로 대체한다.
  문서 변경 자체가 앱의 주 모델을 전환하지는 않는다. 실제 세션 모델과 호출 모델을 확인하고,
  필요한 모델이 없을 때 다른 모델을 같은 이름으로 표시하거나 임의로 역할을 바꾸지 않는다.
  개별 기능의 세부 계획은 배치 시작 시 작성하며, 모든 작은 수정마다 사용자에게 모델 선택을 다시 묻지 않는다.

## 개발 현황과 남은 작업

2026-09-08 인수 확인: 로컬 main `bf357b0`, 원격 main `d8a2cd8`, 운영 Fly v87(정상).
로컬에만 있는 `57eb133`(아트 제작 가이드)·`bf357b0`(스토리 스프라이트 액자 제거)은 아직 push/배포되지 않았다.
R1~R5·Ch1~12·보너스 CG 50종은 구현 완료이며, 아래 과거 배포 시점보다 최신 근거는
`docs/superpowers/handoffs/2026-09-08-astra-orchestration.md`를 따른다. 이 SHA는 인수 작업 시작 시점 기준이다.
...
- 수련 `ObjectiveHud`는 모든 미션 문구·유형·상세 진행도를 기본 인라인으로 표시하고, 헤더의 [미션 접기]/[미션 펼치기]로 스텝 안에서 접고 펼친다.
  상세 진행도는 `formatObjectiveDetailProgress`를 써서 0회 상한·초과 위반 횟수도 보존한다. 졸업 대결의 토너먼트 진행 블록은 유지한다.
...
- R1 완료: Ch3/Ch6 서버 소유 실패 씬과 10분 스파링 재도전 체크포인트, 결산 직후 지갑 갱신,
  320/390px 허브 카드 폭을 구현·검증했다. 스토리 XP 일반 레벨 아이템 누락과 모든 히로인 소급은
  v34 정본 레벨 영수증으로 보정한다. 실제 소켓/별도 QA DB 브라우저 검증은 실행 handoff에 기록한다.
- 아트: SDXL 36후보는 품질 미달, Qwen 첫4+후속12 확대 검수 후 서로 다른8장면 WebP를 신규 공급했다.
  `scripts/art/library-worker.py`는 별도 SQLite 영속 큐·전역 GPU OS lock·intent/metadata 복구·검수 SHA·
  no-overwrite export를 구현했고, 실제 4장 종료→새 프로세스8장 재개를 확인했다. CG8장과 영상2개를
  게임에 연결했다(승리 사쿠라/강변 엘레나 MP4+WebM, 빗소리 사쿠라2후보 반려).
  영상은106프레임·24fps·768×1152, 두 포맷의 원장/해시가 완성된 뒤에만 availability를 등록한다.
  실제 반복 재생·MP4 대체·실패/모션 감소 시 정지 CG 표시를 브라우저에서 확인했다.
  기존 이미지는 교체하지 않는다. 단발/큐 출처는 공급 manifest로 구별한다.
  이후 품질 피드백으로 사쿠라4/엘레나4를 GPT Image2 원화+로컬 H3 루프8개로 재작업해 v2로 연결했다.
  마지막 검증 배포는 `497943a`/Fly v86이며 CgStage 영상 잘림도 수정했다. 이전 파일은 보존했다.
  최신 아트 근거는 `docs/superpowers/handoffs/2026-09-05-event-cg-v2.md`를 따른다.
- R2 완료: Ch7의 관찰12핸드→가면4퀴즈→일괄 정답/정체 공개→최대10핸드 대응 대결.
  `pendingQuiz`는 실제 서버 발급값이며 30초 절대 만료·첫답 불변·최신소켓 소유권을 지킨다. 시험 모드는
  기존 드릴85% 경로를 유지한다. 4막×3개 고정 `curriculum.ts`가 부분 막 공개의 조기 승급을 막는다.
  클라이언트 퀴즈 표시는 서버 remainingMs/sampledAt 표본과 수신 performance.now 경과를 사용한다.
  브라우저 Date.now와 서버 expiresAt을 직접 비교해 버튼을 잠그면 단말 시계 오차로4문 전부 실패할 수
  있으므로 금지. 만료 권위는 서버이며 재전송은 표시 시간을 연장하지 않는다. 필수 퀴즈는 공용 body-portal
  Modal의 dismissible:false로 표시해 작은 화면 스크롤과 오프라인/비활성 시 포커스 유지 계약을 지킨다.
- R3 완료: Ch8·9 씬/레슨/드릴/각2프리셋/12·15핸드 스파링/실패 재도전/보상(v36), 3막 완주 갈색 펠트.
  Ch8 리딩은 공개 카드·가격·명시 레인지만 사용하며 리버의 같은 room/hand/actionSeq 턴을 보류한다.
...
- 후순위: 비참가자 관전(탈락 SnG 관전은 이미 존재), 한 세션 멀티테이블, 추가 계정 연동.
  **프로필·경제·스토리 완료 기록은 SQLite에 영속**, **복구 문구 기반 프로필 복구와 관리자 UI는 구현됨**.
  라이브 방·진행 중 스토리 런의 인메모리 상태 및 단일 서버 전제와 혼동하지 않는다.
