...
def inspect_media(path, kind, expected, meta=None):
    ...
    except (subprocess.SubprocessError,KeyError,TypeError,OverflowError,ZeroDivisionError) as error:
...
def _inspect_media(path, kind, expected, meta=None):
    ...
    if kind=='image':
        ...
        raw=subprocess.check_output(['ffprobe','-v','error','-count_frames','-select_streams','v:0','-show_entries','stream=width,height,nb_read_frames,duration,avg_frame_rate,codec_name:format=duration:format_tags','-of','json',str(path)],stderr=subprocess.STDOUT,timeout=120)
        ...
        result=dict(width=int(stream['width']),height=int(stream['height']),duration=duration,frames=frames,bytes=path.stat().st_size,
            fps=float(Fraction(stream['avg_frame_rate'])),codec=stream['codec_name'])
        for key in ('frames','fps','codec'):
            if key in expected and result[key]!=expected[key]: raise ValueError('Unexpected video '+key)
