...
import { makeChapter, makeChapterChain } from './test-fixtures';
import {
  BLACK_BELT_FLAG,
  beltRank,
...
describe('story unlocks', () => {
  ...
  it('derives belts from fully completed acts and the black-belt flag', () => {
    ...
    expect(deriveBelt(chain, none, {})).toBe('white');
    expect(deriveBelt(chain, new Set(['act1-ch01', 'act1-ch02']), {})).toBe('white');
    expect(deriveBelt(chain, act1, {})).toBe('yellow');
    expect(isActCompleted(chain, 1, act1)).toBe(true);
    expect(isActCompleted(chain, 3, act1)).toBe(false); // 3막 데이터 없음 → 미완료

    const all = new Set(chain.map(c => c.id));
    // 2막은 완료했지만 3·4막 데이터가 없으므로 파란띠에서 멈춘다
    expect(deriveBelt(chain, all, {})).toBe('blue');
    ...
    expect(deriveBelt(fourActs, everything, {})).toBe('brown');
    expect(deriveBelt(fourActs, everything, { [BLACK_BELT_FLAG]: '1' })).toBe('black');
    // 4막을 끝내지 못했으면 플래그가 있어도 검은띠 아님
    expect(deriveBelt(fourActs, new Set([...chain.map(c => c.id), 'act3-ch07']), { [BLACK_BELT_FLAG]: '1' })).toBe('brown');
