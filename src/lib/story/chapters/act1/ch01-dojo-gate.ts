  ...
  steps: [
    ...
    // ───────────────────────────────────────────── 첫 카드 읽기 — 첫 2~3분
    {
      ...
      blocks: [
        ...
        {
          kind: 'concept-card',
          title: '홀카드 2장과 보드 5장 중 가장 좋은 5장',
          body: '내 카드 2장은 홀카드, 모두가 함께 쓰는 공용 카드는 보드예요. 이 중 가장 좋은 5장으로 겨뤄요. A♠ A♦처럼 같은 숫자 둘은 원페어, 두 쌍은 투페어, 셋은 트리플이에요. 5·6·7·8·9처럼 연속된 다섯 장은 스트레이트, 같은 무늬 다섯 장은 플러시예요. 족보가 없으면 가장 높은 카드인 하이카드로 비교해요♪',
          formula: '낮음 → 높음: 하이카드 → 원페어 → 투페어 → 트리플 → 스트레이트 → 플러시 → 풀하우스 → 포카드 → 스트레이트 플러시 → 로열 스트레이트 플러시',
        },
      ],
    },
