...
// 히어로 아바타 원 왼쪽(8시 30분~9시 방향) 가장자리에 살짝 겹치는 좌표 — 유저가 스크린샷에
// 직접 표시한 지점을 역산한 값 (2026-07-22, 아바타 중심 ≈ 50%/86.4% 기준 좌측 -10%/+1.6%).
// 이력: 머리 위 중앙(50%,79%) → 왼쪽 아래(33%,91%) → (40%,92%) → 현재. y를 내리면 이름
// 플레이트 층으로 떨어져 "너무 밑"이 되니 주의. 히어로 홀카드는 오른쪽 부착이라 왼쪽이 빈다.
const LAUNCHER_POS: TablePos = { x: '40%', y: '88%' };
