...
describe('PokerDatabase migrations', () => {
  ...
  it('adds the V40 admin trusted device table without raw credential columns', () => {
    database = openPokerDatabase(':memory:');

    expect(database.tableNames()).toEqual(
      expect.arrayContaining(['admin_trusted_devices']),
    );
    const columns = database.db
      .prepare('PRAGMA table_info(admin_trusted_devices)')
      .all()
    ...
    `).all().map(row => (row as { name: string }).name)).toEqual(
      expect.arrayContaining(['idx_admin_trusted_devices_scope']),
    );
    ...
    const insert = (
      ...
      ) VALUES (?, ?, 'scope', 'csrf', 'admin_public', ?, ?, ?, ?)
    `).run(id, lookupKey, name, createdAt, createdAt, expiresAt);

    insert('device-1', 'lookup-1', '등록된 브라우저', 1_000, 1_000 + 90 * 86_400_000);
    // 같은 조회 키는 유일하다 — 자격 하나에 행 하나.
    expect(() => insert('device-2', 'lookup-1', '중복', 1_000, 2_000)).toThrow();
    // 빈 이름·80자 초과·거꾸로 된 만료는 스키마에서 막는다.
    expect(() => insert('device-3', 'lookup-3', '', 1_000, 2_000)).toThrow();
    expect(() => insert('device-4', 'lookup-4', 'x'.repeat(81), 1_000, 2_000))
      .toThrow();
    expect(() => insert('device-5', 'lookup-5', '이름', 1_000, 1_000)).toThrow();
  });
  ...
  it('applies the initial schema migration', () => {
    ...
    expect(migration.version).toBe(40);
    expect(database.tableNames()).toEqual(
  ...
  it('adds the v32 story reward catalog, receipts, inventory sync and cosmetic tables', () => {
    ...
    // v39 보너스 CG 50행 — 히로인 30행은 character_id, 비히로인 20행은 NULL(CHECK 목록 밖이라 NULL이 유일한 값)
    const bonus = seeded.filter(row => String(row.item_id).startsWith('story-bonus-cg-'));
    expect(bonus).toHaveLength(50);
    expect(bonus.every(row => row.kind === 'cg' && row.equip_slot === null && row.chip_amount === null)).toBe(true);
    expect(bonus.filter(row => row.character_id === null)).toHaveLength(20);
    for (const subject of ['miyako', 'yuzuki', 'lin', 'ingrid']) {
      expect(bonus.filter(row => String(row.item_id).startsWith(`story-bonus-cg-${subject}-`)).every(row => row.character_id === null)).toBe(true);
    }
    expect(bonus.find(row => row.item_id === 'story-bonus-cg-sakura-beach')?.character_id).toBe('sakura');
    expect(() => database?.db.prepare(`
  ...
  it('adds v27 scheduled tournament and promotion fund schema', () => {
    ...
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      SELECT first_starts_at, recurrence_ends_at
      FROM tournament_template
      WHERE id = 'imported-template'
    `).get()).toEqual({
      first_starts_at: null,
      recurrence_ends_at: null,
    });
  });

  it('disables legacy recurring templates until an explicit end is set', () => {
    const directory = mkdtempSync(join(tmpdir(), 'poker-doku-'));
    temporaryDirectories.push(directory);
    const path = join(directory, 'poker.sqlite');
    createDatabaseThroughMigration(path, 28);
    ...
    raw.prepare(`
      INSERT INTO tournament_template (
        ...
        'legacy-recurring', 1, 'legacy-recurring-key', 'Legacy', 1,
        'Asia/Seoul', '{"kind":"hourly","minute":0}', 3600000, 1200000,
        2, '{}', 'legacy-import', NULL, 1, 1
      )
    `).run();
    raw.close();
  ...
  it('keeps ledgers settlement results and terminal identities immutable', () => {
    ...
    fundAndReserveTournamentPrize(database.db, 'immutable-instance', 100);
    database.db.exec(`
      UPDATE tournament_instance
    ...
    database.db.exec(`
      ...
      SET status = 'running', registration_state = 'closed',
          ...
          initial_entrants = 1, initial_bot_entrants = 0,
          committed_entrants = 1, final_entrants = 1,
          payout_freeze_version = 1, payout_freeze_json = '{}',
    ...
    database.db.exec(`
      ...
      SET settlement_fingerprint = 'fingerprint-1', updated_at = 20
    ...
    expect(() => database?.db.exec(`
      ...
      WHERE id = 'reserve-ledger:immutable-instance'
    `)).toThrow();
    expect(() => database?.db.exec(`
  ...
  it('allows only one active mtt claim per profile', () => {
    database = openPokerDatabase(':memory:');
    for (const instanceId of ['claim-a', 'claim-b']) {
      insertTournamentInstance(database.db, {
        id: instanceId,
        status: 'scheduled-hidden',
        registrationState: 'not-open',
      ...
      fundAndReserveTournamentPrize(database.db, instanceId, 100);
      database.db.exec(`
  ...
  it('rebuilds v28 sng entries with attempt generations without losing v26 rows', () => {
    ...
    database = openPokerDatabase(path);
    expect(database.db.prepare(`
      SELECT id, tournament_id, room_id, profile_id, buy_in, fee, status,
    ...
    `).get()).toEqual({
      ...
      room_id: 'tournament-a',
      profile_id: 'attempt-profile',
      buy_in: 1500,
      fee: 150,
      entry_attempt: 1,
      status: 'refunded',
    ...
    const activeIndex = database.db.prepare(`
      SELECT sql FROM sqlite_schema
      WHERE type = 'index' AND name = 'one_active_sng_entry_per_profile'
    `).get() as { sql: string };
    expect(activeIndex.sql.replace(/\s+/g, ' ')).toContain(
    ...
    `).run()).toThrow();
    expect(database.db.prepare(`
      DELETE FROM profiles WHERE id = 'attempt-profile'
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      SELECT COUNT(*) AS count FROM sng_entries
      WHERE profile_id = 'attempt-profile'
    `).get()).toEqual({ count: 0 });
  });
  ...
  it('rolls all v27 and v28 objects back when a middle statement fails', () => {
    ...
    rawDatabase.exec(`
      INSERT INTO profiles (
        ...
        'rollback-profile', 'rollback-hash', 'rollback-lookup',
        'rollback-recovery', 'rollback-recovery-lookup',
        'rollback-alias', 'sakura', 1, 1, 1
      );
      INSERT INTO sng_entries (
        id, tournament_id, room_id, profile_id, buy_in, fee, status,
        place, prize, start_attempt, created_at, updated_at
      ) VALUES (
        'rollback-entry', 'rollback-tournament', 'rollback-room',
        'rollback-profile', 1500, 150, 'refunded', NULL, 0, 0, 1, 1
      );
      DROP INDEX one_active_sng_entry_per_profile;
      CREATE UNIQUE INDEX one_active_sng_entry_per_profile ON profiles(id);
    `);
    rawDatabase.close();

    expect(() => openPokerDatabase(path)).toThrowError(
      'index one_active_sng_entry_per_profile already exists',
    );
    ...
    try {
      ...
      expect(reopened.prepare(`
        SELECT id, tournament_id, profile_id FROM sng_entries
        WHERE id = 'rollback-entry'
      `).get()).toEqual({
        id: 'rollback-entry',
        tournament_id: 'rollback-tournament',
        profile_id: 'rollback-profile',
      });
      expect(reopened.prepare(`
        SELECT COUNT(*) AS count FROM sqlite_schema
        WHERE type = 'table' AND name = 'sng_entries_v26_backup'
      `).get()).toEqual({ count: 0 });
    } finally {
      reopened.close();
    }
  ...
  it('enforces registration state generation and close-owner CAS transitions', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'registration-cas',
      status: 'running',
    ...
    expect(database.db.prepare(`
      ...
      SET registration_state = 'closing',
          ...
          registration_generation = 1,
          registration_owner_token = 'close-owner-1'
      WHERE id = 'registration-cas'
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET registration_owner_token = 'close-owner-2'
      WHERE id = 'registration-cas'
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET registration_close_reason = 'full',
          registration_generation = 2,
          registration_owner_token = 'close-owner-2'
      WHERE id = 'registration-cas'
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET registration_generation = 0
      WHERE id = 'registration-cas'
    `).run()).toThrow();
    expect(database.db.prepare(`
      UPDATE tournament_instance
    ...
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET registration_state = 'open-late',
    ...
    expect(() => database?.db.prepare(`
      ...
      SET registration_state = 'closed',
          registration_close_reason = 'time'
      WHERE id = 'registration-skip'
    `).run()).toThrow();
  });

  it('supports scheduler catch-up but blocks cancellation with live liabilities', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'catch-up-registering',
      status: 'scheduled-hidden',
      registrationState: 'not-open',
    });
    expect(database.db.prepare(`
    ...
    expect(database.db.prepare(`
      ...
      WHERE id = 'catch-up-delayed'
    `).run().changes).toBe(1);

    insertProfile(database, 'liability-profile');
    insertTournamentInstance(database.db, {
      id: 'wallet-liability',
      status: 'registering',
      registrationState: 'open-prestart',
    ...
    database.db.exec(`
      INSERT INTO sng_entries (
        ...
        'liability-entry', 'wallet-liability', 'wallet-liability',
        'liability-profile', 1500, 150, 'reserved',
        NULL, 0, 0, 1, 10, 10
      );
    `);
    expect(() => database?.db.prepare(`
    ...
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET status = 'cancelled',
          status_reason = 'not-enough',
          registration_state = 'closed',
          registration_close_reason = 'tournament-cancelled',
          registration_generation = 1,
          completed_at = 20
      WHERE id = 'unsafe-hidden-cancel'
    `).run()).toThrow();
  });

  it('keeps terminal registration attempts and close claims immutable', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, { id: 'history-instance' });
    prepareWalletEconomyEntry(
      database.db, 'history-instance', 'history-profile', 1,
    );
    prepareWalletEconomyEntry(
      database.db, 'history-instance', 'close-profile', 1,
    );
    database.db.exec(`
      INSERT INTO tournament_registration (
        instance_id, profile_id, public_player_json, status, ever_seated,
        ...
        'history-instance', 'history-profile', '{}', 'registered',
        0, 1, 1, 10, 10
      );
      INSERT INTO tournament_registration_attempt (
        ...
        1, 'registered', NULL, NULL, NULL, 10, 10
      );
    ...
    expect(() => database?.db.prepare(`
      UPDATE tournament_registration
      SET status = 'registered', updated_at = 30
      WHERE instance_id = 'history-instance' AND profile_id = 'history-profile'
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
    ...
    `).run()).toThrow();
    database.db.prepare(`
      UPDATE sng_entries SET status = 'refunded', updated_at = 30
      WHERE tournament_id = 'history-instance'
        AND profile_id = 'history-profile' AND entry_attempt = 1
    `).run();
    prepareWalletEconomyEntry(
      database.db, 'history-instance', 'history-profile', 2,
    );
    expect(database.db.prepare(`
      INSERT INTO tournament_registration_attempt (
        instance_id, profile_id, registration_attempt, request_id,
        ...
        2, 'registered', NULL, NULL, NULL, 30, 30
      )
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      UPDATE tournament_registration
      SET status = 'registered', registration_attempt = 2,
          economy_entry_attempt = 2, registered_at = 30, updated_at = 30
      WHERE instance_id = 'history-instance' AND profile_id = 'history-profile'
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
      UPDATE tournament_registration_attempt
    ...
    database.db.exec(`
      INSERT INTO tournament_registration (
        ...
        'history-instance', 'close-profile', '{}', 'late-pending',
        0, 1, 1, 10, 10
      );
      INSERT INTO tournament_registration_attempt (
        instance_id, profile_id, registration_attempt, request_id,
        ...
        1, 'late-pending', NULL, NULL, NULL, 10, 10
      );
    `);
    expect(() => database?.db.prepare(`
      UPDATE tournament_registration_attempt
      SET close_owner_token = 'different-owner'
      WHERE instance_id = 'history-instance' AND profile_id = 'close-profile'
        AND registration_attempt = 1
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_registration (
        ...
        'history-instance', 'terminal-insert', '{}', 'refunded',
        0, 1, NULL, 10, 10
      )
    `).run()).toThrow();
  });

  it('requires freeroll escrow before public and terminal lifecycle states', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'unfunded-freeroll',
      status: 'scheduled-hidden',
      registrationState: 'not-open',
      economyMode: 'freeroll',
    });
    ...
    fundAndReserveTournamentPrize(database.db, 'funded-freeroll', 100);
    database.db.exec(`
      UPDATE tournament_instance
      SET status = 'scheduled-visible'
      WHERE id = 'funded-freeroll';
      UPDATE tournament_instance
      SET status = 'registering', registration_state = 'open-prestart'
      WHERE id = 'funded-freeroll';
    `);
    ...
    expect(() => database?.db.prepare(`
      UPDATE tournament_prize_escrow
      SET status = 'refunded', refunded_at = 30, updated_at = 30
      WHERE instance_id = 'funded-freeroll'
    `).run()).toThrow();
    expect(database.db.prepare(`
      INSERT INTO promotion_fund_ledger (
        ...
        'refund-ledger:funded-freeroll', 'global',
        'freeroll-prize-refund', 100, 100, 'funded-freeroll',
        'system', 'scheduler', 'refund prize',
        'refund-key:funded-freeroll', 30
      )
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      UPDATE tournament_prize_escrow
  ...
  it('rejects direct terminal escrow and settlement inserts', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'terminal-escrow-insert',
      status: 'scheduled-hidden',
      registrationState: 'not-open',
      economyMode: 'freeroll',
    });
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_prize_escrow (
        ...
        'terminal-escrow-insert', 'global', 100, 'refunded', 0, 0,
        NULL, 10, NULL, 10, 10
      )
    `).run()).toThrow();

    insertTournamentInstance(database.db, {
      id: 'terminal-settlement-insert',
      status: 'running',
      registrationState: 'closed',
      registrationCloseReason: 'late-reg-disabled',
      entrantCount: 1,
    ...
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_settlement (
        ...
        'terminal-settlement-insert', 'settled', 'checksum', 1,
        100, 100, 0, 'terminal-fingerprint', 10, 10, 10
      )
    `).run()).toThrow();
  });

  it('requires a complete contiguous and mode-correct settlement plan', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'non-contiguous-plan',
      status: 'running',
      registrationState: 'open-late',
    });
    insertFinishedTournamentRegistration(
      database.db, 'non-contiguous-plan', 'profile-1', 'non-contiguous-1',
    );
    insertFinishedTournamentRegistration(
      database.db, 'non-contiguous-plan', 'profile-2', 'non-contiguous-2',
    );
    freezeRunningTournament(database.db, 'non-contiguous-plan');
    database.db.exec(`
      INSERT INTO tournament_settlement VALUES (
        'non-contiguous-plan', 'pending', 'checksum', 2,
        100, 100, 0, 'non-contiguous-fingerprint', 10, NULL, 10
      );
      INSERT INTO tournament_settlement_result VALUES
        ('non-contiguous-plan', 1, 'player-1', 'human', 'profile-1',
          1, 'One', 60, 'wallet-credit'),
        ('non-contiguous-plan', 3, 'player-2', 'human', 'profile-2',
          1, 'Two', 40, 'wallet-credit');
    `);
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET status = 'payout-pending'
      WHERE id = 'non-contiguous-plan'
    `).run()).toThrow();
    ...
    insertFinishedTournamentRegistration(
      database.db, 'wrong-sum-plan', 'profile-a', 'wrong-sum-a',
    );
    insertFinishedTournamentRegistration(
      database.db, 'wrong-sum-plan', 'profile-b', 'wrong-sum-b',
    );
    freezeRunningTournament(database.db, 'wrong-sum-plan');
    database.db.exec(`
      INSERT INTO tournament_settlement VALUES (
        'wrong-sum-plan', 'pending', 'checksum', 2,
        100, 100, 0, 'wrong-sum-fingerprint', 10, NULL, 10
      );
      INSERT INTO tournament_settlement_result VALUES
        ('wrong-sum-plan', 1, 'player-a', 'human', 'profile-a',
          1, 'A', 60, 'wallet-credit'),
        ('wrong-sum-plan', 2, 'player-b', 'human', 'profile-b',
          1, 'B', 30, 'wallet-credit');
    `);
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET status = 'payout-pending'
      WHERE id = 'wrong-sum-plan'
    `).run()).toThrow();
    ...
    insertFinishedTournamentRegistration(
      database.db, 'wallet-bot-plan', 'wallet-profile', 'wallet-bot-human',
    );
    freezeRunningTournament(database.db, 'wallet-bot-plan');
    database.db.exec(`
      INSERT INTO tournament_settlement VALUES (
        'wallet-bot-plan', 'pending', 'checksum', 2,
        100, 50, 50, 'wallet-bot-fingerprint', 10, NULL, 10
      );
      INSERT INTO tournament_settlement_result VALUES
        ('wallet-bot-plan', 1, 'wallet-human', 'human', 'wallet-profile',
          1, 'Human', 50, 'wallet-credit'),
        ('wallet-bot-plan', 2, 'wallet-bot', 'bot', NULL,
          NULL, 'Bot', 50, 'promotion-return');
    `);
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET status = 'payout-pending'
      WHERE id = 'wallet-bot-plan'
    `).run()).toThrow();
  });

  it('couples runtime timestamps and owners to immutable lifecycle fields', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'early-start-time',
      status: 'registering',
    ...
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET actual_started_at = 10
      WHERE id = 'early-start-time'
    `).run()).toThrow();
    ...
    expect(database.db.prepare(`
      ...
      SET status = 'running', registration_state = 'open-late',
          initial_entrants = 2, initial_bot_entrants = 0,
          committed_entrants = 2, actual_started_at = 20,
          start_owner_id = NULL, start_lease_until = NULL
      WHERE id = 'starting-owner'
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET initial_entrants = 3
      WHERE id = 'starting-owner'
    `).run()).toThrow();
    ...
    expect(() => database?.db.prepare(`
      ...
      SET status = 'cancelled',
          ...
          registration_close_reason = 'tournament-cancelled',
          registration_generation = 1
      WHERE id = 'terminal-time'
    `).run()).toThrow();
  });

  it('reconciles every promotion ledger write with the protected fund balance', () => {
    database = openPokerDatabase(':memory:');
    expect(() => database?.db.prepare(`
      UPDATE promotion_fund
      SET balance = 100, version = 1, updated_at = 10
    ...
    expect(() => database?.db.prepare(`
      INSERT INTO promotion_fund_ledger (
        ...
        'bad-balance-ledger', 'global', 'admin-adjustment', 100, 99, NULL,
        'backoffice-admin', 'admin-1', 'fund seed',
        'bad-balance-key', 10
      )
    `).run()).toThrow();
    expect(database.db.prepare(`
      INSERT INTO promotion_fund_ledger (
        ...
        'fund-ledger-1', 'global', 'admin-adjustment', 100, 100, NULL,
        'backoffice-admin', 'admin-1', 'fund seed',
        'fund-ledger-key-1', 10
      )
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      SELECT balance, version, updated_at FROM promotion_fund
      WHERE account_id = 'global'
    `).get()).toEqual({ balance: 100, version: 1, updated_at: 10 });
    expect(() => database?.db.prepare(`
      INSERT INTO promotion_fund_ledger (
        id, account_id, kind, delta, balance_after, instance_id,
        actor_kind, actor_id, reason, idempotency_key, created_at
      ) VALUES (
        'orphan-reserve', 'global', 'freeroll-prize-reserve', -10, 90,
        'missing-instance', 'system', 'scheduler', 'orphan reserve',
        'orphan-reserve-key', 15
      )
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      INSERT INTO promotion_fund_ledger (
        ...
        'overdraw-ledger', 'global', 'admin-adjustment', -101, 0, NULL,
        'backoffice-admin', 'admin-1', 'bad debit',
        'overdraw-key', 20
      )
    `).run()).toThrow();
  });

  it('locks settlement results to pending plans and matching registrations', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'result-lock-instance',
      status: 'running',
      registrationState: 'open-late',
      entrantCount: 1,
    });
    insertFinishedTournamentRegistration(
      database.db, 'result-lock-instance', 'result-profile', 'result-request',
    );
    freezeRunningTournament(database.db, 'result-lock-instance');
    database.db.exec(`
      INSERT INTO tournament_settlement VALUES (
        'result-lock-instance', 'pending', 'checksum', 1,
        100, 100, 0, 'result-lock-fingerprint', 10, NULL, 10
      );
    `);
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_settlement_result VALUES (
        'result-lock-instance', 1, 'wrong-player', 'human', 'wrong-profile',
        1, 'Wrong', 100, 'wallet-credit'
      )
    `).run()).toThrow();
    expect(database.db.prepare(`
      INSERT INTO tournament_settlement_result VALUES (
        'result-lock-instance', 1, 'right-player', 'human', 'result-profile',
        1, 'Right', 100, 'wallet-credit'
      )
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
      UPDATE tournament_settlement
      SET status = 'settled', settled_at = 20, updated_at = 20
      WHERE instance_id = 'result-lock-instance'
    `).run()).toThrow();
    expect(database.db.prepare(`
      UPDATE tournament_instance SET status = 'payout-pending'
      WHERE id = 'result-lock-instance'
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      UPDATE tournament_settlement
      SET status = 'settled', settled_at = 20, updated_at = 20
    ...
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_settlement_result VALUES (
        'result-lock-instance', 2, 'late-player', 'bot', NULL,
        NULL, 'Late', 0, 'none'
      )
    `).run()).toThrow();
  });

  it('rejects terminal instance insertion and freezes monotonic audit fields', () => {
    database = openPokerDatabase(':memory:');
    expect(() => insertTournamentInstance(database!.db, {
      id: 'inserted-cancelled',
      status: 'cancelled',
      registrationState: 'closed',
      registrationCloseReason: 'tournament-cancelled',
    })).toThrow();
    expect(() => insertTournamentInstance(database!.db, {
      id: 'inserted-public-freeroll',
      status: 'running',
      registrationState: 'open-late',
      economyMode: 'freeroll',
    })).toThrow();
    ...
    insertTournamentInstance(database.db, {
      id: 'monotonic-instance',
      status: 'registering',
      registrationState: 'open-prestart',
    });
    expect(database.db.prepare(`
      UPDATE tournament_instance
      SET start_attempt = 2, settlement_attempt = 2, forfeited_chips = 100
      WHERE id = 'monotonic-instance'
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
    ...
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET settlement_attempt = 1
      WHERE id = 'monotonic-instance'
    `).run()).toThrow();
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance SET forfeited_chips = 99
      WHERE id = 'monotonic-instance'
    `).run()).toThrow();
  });

  it('validates contractual JSON and prevents registration history deletion', () => {
    database = openPokerDatabase(':memory:');
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_template (
        id, revision, idempotency_key, name, enabled, timezone,
        ...
        'invalid-json-template', 1, 'invalid-json-key', 'Invalid', 1,
        'Asia/Seoul', '{', 1, 1, 2, '{}', 'system', NULL, 1, 1
      )
    `).run()).toThrow();
    expect(() => insertTournamentInstance(database!.db, {
      id: 'invalid-json-instance',
      status: 'registering',
      registrationState: 'open-prestart',
    }, '{')).toThrow();

    insertTournamentInstance(database.db, { id: 'delete-history-instance' });
    prepareWalletEconomyEntry(
      database.db, 'delete-history-instance', 'delete-profile', 1,
    );
    database.db.exec(`
      INSERT INTO tournament_registration (
        instance_id, profile_id, public_player_json, status, ever_seated,
        ...
        'delete-history-instance', 'delete-profile', '{}', 'registered',
        0, 1, 1, 10, 10
      );
      INSERT INTO tournament_registration_attempt (
        instance_id, profile_id, registration_attempt, request_id,
        ...
        'delete-history-instance', 'delete-profile', 1, 'delete-request',
        1, 'registered', NULL, NULL, NULL, 10, 10
      );
    `);
    expect(() => database?.db.prepare(`
  ...
  it('synchronizes registration attempts and validates economy links', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, { id: 'ordered-history' });
    prepareWalletEconomyEntry(
      database.db, 'ordered-history', 'ordered-profile', 1,
    );
    database.db.exec(`
  ...
  it('ties stored close claims to the live instance close owner', () => {
    ...
    prepareWalletEconomyEntry(
      database.db, 'claim-owner-instance', 'claim-profile', 1,
    );
    database.db.exec(`
      INSERT INTO tournament_registration VALUES (
      ...
      SET registration_state = 'closing',
          ...
          registration_generation = 1,
          registration_owner_token = 'instance-owner'
      WHERE id = 'claim-owner-instance';
    `);
    expect(() => database?.db.prepare(`
      UPDATE tournament_registration_attempt
  ...
  it('freezes registration identity within one attempt', () => {
    ...
    prepareWalletEconomyEntry(
      database.db, 'registration-identity', 'identity-profile', 1,
    );
    database.db.exec(`
      INSERT INTO tournament_registration VALUES (
  ...
  it('retains recent terminal registration history', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, { id: 'old-terminal-parent' });
    prepareWalletEconomyEntry(
      database.db, 'old-terminal-parent', 'old-profile', 1,
    );
    database.db.exec(`
      INSERT INTO tournament_registration VALUES (
      ...
      UPDATE sng_entries SET status = 'refunded', updated_at = 20
      WHERE tournament_id = 'old-terminal-parent'
        AND profile_id = 'old-profile';
      UPDATE tournament_instance
      SET status = 'cancelled', status_reason = 'operator-cancel',
          registration_state = 'closed',
          registration_close_reason = 'tournament-cancelled',
  ...
  it('requires generation-matched wallet entries before completion', () => {
    ...
    `);
    freezeRunningTournament(database.db, 'wallet-generation');
  ...
  it('allows committed entrants to grow after the running snapshot', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
      id: 'late-commit-growth',
      status: 'running',
      registrationState: 'open-late',
      entrantCount: 2,
    });
    expect(database.db.prepare(`
      UPDATE tournament_instance
      SET committed_entrants = 3, updated_at = 20
      WHERE id = 'late-commit-growth'
    `).run().changes).toBe(1);
    expect(database.db.prepare(`
      SELECT initial_entrants, committed_entrants
      FROM tournament_instance WHERE id = 'late-commit-growth'
    `).get()).toEqual({ initial_entrants: 2, committed_entrants: 3 });
  });

  it('atomically closes open late registration when cancellation begins', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
    ...
    expect(database.db.prepare(`
      ...
      SET status = 'refund-pending',
          ...
          registration_generation = 1,
          updated_at = 20
      WHERE id = 'atomic-cancellation'
    `).run().changes).toBe(1);
  });

  it('keeps attempt history aligned when an active-claim update loses contention', () => {
    database = openPokerDatabase(':memory:');
    for (const instanceId of ['claim-sync-a', 'claim-sync-b']) {
      insertTournamentInstance(database.db, {
        id: instanceId,
        status: 'scheduled-hidden',
        registrationState: 'not-open',
      ...
      database.db.prepare(`
        INSERT INTO tournament_registration VALUES (
          ?, 'contended-profile', '{}', 'registered',
          0, 1, NULL, 10, 10
        )
      `).run(instanceId);
      database.db.prepare(`
        INSERT INTO tournament_registration_attempt VALUES (
          ?, 'contended-profile', 1, ?, NULL, 'registered',
          NULL, NULL, NULL, 10, 10
        )
      `).run(instanceId, `${instanceId}-request`);
    }
    expect(database.db.prepare(`
      UPDATE tournament_registration SET status = 'seat-claimed'
      WHERE instance_id = 'claim-sync-a' AND profile_id = 'contended-profile'
    `).run().changes).toBe(1);
    expect(() => database?.db.prepare(`
      UPDATE tournament_registration SET status = 'seat-claimed'
      WHERE instance_id = 'claim-sync-b' AND profile_id = 'contended-profile'
    `).run()).toThrow();
    expect(database.db.prepare(`
      ...
      WHERE registration.instance_id = 'claim-sync-b'
        AND registration.profile_id = 'contended-profile'
    `).get()).toEqual({
  ...
  it('requires a live generation-matched wallet entry at registration', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, { id: 'wallet-link-registration' });
    for (const [profileId, entryStatus, entryAttempt] of [
      ['wallet-null-profile', 'reserved', 1],
      ['wallet-refunded-profile', 'refunded', 1],
      ['wallet-settled-profile', 'settled', 1],
      ['wallet-mismatch-profile', 'reserved', 2],
      ['wallet-valid-profile', 'reserved', 3],
    ] as const) {
      ...
      database.db.prepare(`
        INSERT INTO sng_entries (
          id, tournament_id, room_id, profile_id, buy_in, fee, status,
          place, prize, start_attempt, entry_attempt, created_at, updated_at
        ) VALUES (?, 'wallet-link-registration', 'wallet-link-registration',
          ?, 1500, 150, ?, NULL, 0, 0, ?, 10, 10)
      `).run(`${profileId}-entry`, profileId, entryStatus, entryAttempt);
    }
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_registration VALUES (
        'wallet-link-registration', 'wallet-null-profile', '{}', 'registered',
        0, 1, NULL, 10, 10
      )
    `).run()).toThrow();
    for (const profileId of [
      'wallet-refunded-profile', 'wallet-settled-profile',
    ]) {
      expect(() => database?.db.prepare(`
        INSERT INTO tournament_registration VALUES (
          'wallet-link-registration', ?, '{}', 'registered',
          0, 1, 1, 10, 10
        )
    ...
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_registration VALUES (
        'wallet-link-registration', 'wallet-mismatch-profile', '{}',
        'registered', 0, 1, 1, 10, 10
      )
    `).run()).toThrow();
    expect(database.db.prepare(`
      INSERT INTO tournament_registration VALUES (
        'wallet-link-registration', 'wallet-valid-profile', '{}',
        'registered', 0, 1, 3, 10, 10
      )
    `).run().changes).toBe(1);
  });

  it('accepts registrations only while the lifecycle is open', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, {
    ...
    insertProfile(database, 'hidden-registration-profile');
    database.db.exec(`
      INSERT INTO sng_entries (
        id, tournament_id, room_id, profile_id, buy_in, fee, status,
        place, prize, start_attempt, entry_attempt, created_at, updated_at
      ) VALUES (
        'hidden-registration-entry', 'hidden-registration',
        'hidden-registration', 'hidden-registration-profile',
        1500, 150, 'reserved', NULL, 0, 0, 1, 10, 10
      );
    `);
    expect(() => database?.db.prepare(`
      INSERT INTO tournament_registration VALUES (
  ...
  it('rejects backdated terminal timestamps that bypass retention', () => {
    database = openPokerDatabase(':memory:');
    insertTournamentInstance(database.db, { id: 'backdated-terminal' });
    expect(() => database?.db.prepare(`
      UPDATE tournament_instance
      SET status = 'cancelled', status_reason = 'operator-cancel',
          registration_state = 'closed',
      ...
      WHERE id = 'backdated-terminal'
    `).run()).toThrow();
  });

  it('purges an aged terminal aggregate while retaining economy ledgers', () => {
    database = openPokerDatabase(':memory:');
    insertProfile(database, 'retained-wallet-profile');
    insertTournamentInstance(database.db, {
      id: 'aged-terminal-aggregate',
      status: 'scheduled-hidden',
      registrationState: 'not-open',
      economyMode: 'freeroll',
      entrantCount: 1,
    });
    fundAndReserveTournamentPrize(database.db, 'aged-terminal-aggregate', 100);
    database.db.exec(`
      UPDATE tournament_instance SET status = 'scheduled-visible'
      WHERE id = 'aged-terminal-aggregate';
      UPDATE tournament_instance
      SET status = 'registering', registration_state = 'open-prestart'
      WHERE id = 'aged-terminal-aggregate';
      INSERT INTO tournament_registration VALUES (
        'aged-terminal-aggregate', 'retained-wallet-profile', '{}',
        'registered', 0, 1, NULL, 10, 10
      );
      INSERT INTO tournament_registration_attempt VALUES (
        'aged-terminal-aggregate', 'retained-wallet-profile', 1,
        'aged-terminal-request', NULL, 'registered',
        NULL, NULL, NULL, 10, 10
      );
      DROP TRIGGER protect_tournament_registration_history;
      DROP TRIGGER protect_tournament_registration_attempt_history;
      DROP TRIGGER IF EXISTS sync_tournament_registration_attempt_status;
      UPDATE tournament_registration
      SET status = 'finished', ever_seated = 1, updated_at = 13
      WHERE instance_id = 'aged-terminal-aggregate';
      UPDATE tournament_registration_attempt
      SET status = 'finished', updated_at = 13
      WHERE instance_id = 'aged-terminal-aggregate';
      UPDATE tournament_instance
      SET status = 'starting', registration_state = 'locked-for-start',
          start_attempt = 1, start_owner_id = 'retention-owner',
          start_lease_until = 100
      WHERE id = 'aged-terminal-aggregate';
      UPDATE tournament_instance
      SET status = 'running', registration_state = 'closed',
      ...
      WHERE id = 'aged-terminal-aggregate';
      INSERT INTO tournament_forfeit VALUES (
        'aged-forfeit', 'aged-terminal-aggregate', 'removed-player',
        NULL, NULL, 0, 1, NULL, 20
      );
      INSERT INTO tournament_settlement VALUES (
        'aged-terminal-aggregate', 'pending', 'aged-checksum', 1,
        100, 100, 0, 'aged-fingerprint', 20, NULL, 20
      );
      INSERT INTO tournament_settlement_result VALUES (
        'aged-terminal-aggregate', 1, 'aged-player', 'human',
        'retained-wallet-profile', 1, 'Aged Player', 100, 'wallet-credit'
      );
      UPDATE tournament_prize_escrow
      SET settlement_fingerprint = 'aged-fingerprint', updated_at = 20
      WHERE instance_id = 'aged-terminal-aggregate';
      UPDATE tournament_instance SET status = 'payout-pending'
      WHERE id = 'aged-terminal-aggregate';
      UPDATE tournament_prize_escrow
      SET status = 'settled', human_paid = 100, bot_returned = 0,
          settled_at = 30, updated_at = 30
      WHERE instance_id = 'aged-terminal-aggregate';
      UPDATE tournament_settlement
      SET status = 'settled', settled_at = 30, updated_at = 30
      WHERE instance_id = 'aged-terminal-aggregate';
      DROP TRIGGER IF EXISTS validate_tournament_terminal_timestamp;
      UPDATE tournament_instance
      SET status = 'completed', completed_at = 10
      WHERE id = 'aged-terminal-aggregate';
      INSERT INTO sng_entries (
        id, tournament_id, room_id, profile_id, buy_in, fee, status,
        place, prize, start_attempt, entry_attempt, created_at, updated_at
      ) VALUES (
        'retained-wallet-ledger', 'aged-terminal-aggregate',
        'aged-terminal-aggregate', 'retained-wallet-profile',
        1500, 150, 'settled', 1, 100, 1, 1, 10, 30
      );
    `);
    expect(database.db.prepare(`
      DELETE FROM tournament_instance WHERE id = 'aged-terminal-aggregate'
    `).run().changes).toBe(1);
    for (const table of [
      'tournament_registration_attempt',
      'tournament_registration',
    ...
    expect(database.db.prepare(`
      SELECT COUNT(*) AS count FROM promotion_fund_ledger
      WHERE instance_id = 'aged-terminal-aggregate'
    `).get()).toEqual({ count: 1 });
    expect(database.db.prepare(`
      SELECT COUNT(*) AS count FROM sng_entries
      WHERE tournament_id = 'aged-terminal-aggregate'
    `).get()).toEqual({ count: 1 });
  });
  ...
  it('upgrades an existing V1 database through the latest schema once while preserving data', () => {
    ...
    expect(versions).toEqual([
      ...
      { version: 30 }, { version: 31 }, { version: 32 }, { version: 33 }, { version: 34 }, { version: 35 }, { version: 36 }, { version: 37 }, { version: 38 }, { version: 39 }, { version: 40 },
    ]);
...
function fundAndReserveTournamentPrize(
  target: DatabaseSync,
  instanceId: string,
  amount: number,
): void {
  const { balance } = target.prepare(`
    SELECT balance FROM promotion_fund WHERE account_id = 'global'
  ...
  target.exec(`
    INSERT INTO promotion_fund_ledger (
      ...
      'adjust-ledger:${instanceId}', 'global', 'admin-adjustment',
      ${amount}, ${balance + amount}, NULL,
      'backoffice-admin', 'test-admin', 'fund prize',
      'adjust-key:${instanceId}', 5
    );
    INSERT INTO promotion_fund_ledger (
      id, account_id, kind, delta, balance_after, instance_id,
      actor_kind, actor_id, reason, idempotency_key, created_at
    ) VALUES (
      'reserve-ledger:${instanceId}', 'global', 'freeroll-prize-reserve',
      -${amount}, ${balance}, '${instanceId}',
      'system', 'scheduler', 'reserve prize',
      'reserve-key:${instanceId}', 10
    );
    INSERT INTO tournament_prize_escrow (
      instance_id, account_id, amount, status, human_paid, bot_returned,
      settlement_fingerprint, reserved_at, settled_at, refunded_at, updated_at
...
function insertFinishedTournamentRegistration(
  target: DatabaseSync,
  instanceId: string,
  profileId: string,
  requestId: string,
): void {
  const instance = target.prepare(`
    SELECT economy_mode FROM tournament_instance WHERE id = ?
  `).get(instanceId) as { economy_mode: 'freeroll' | 'wallet' };
  const economyEntryAttempt = instance.economy_mode === 'wallet' ? 1 : null;
  if (economyEntryAttempt !== null) {
    prepareWalletEconomyEntry(
      target, instanceId, profileId, economyEntryAttempt,
    );
  }
  target.prepare(`
    INSERT INTO tournament_registration (
      instance_id, profile_id, public_player_json, status, ever_seated,
      registration_attempt, economy_entry_attempt, registered_at, updated_at
    ) VALUES (?, ?, '{}', 'registered', 0, 1, ?, 10, 10)
  `).run(instanceId, profileId, economyEntryAttempt);
  target.prepare(`
    INSERT INTO tournament_registration_attempt (
      instance_id, profile_id, registration_attempt, request_id,
      economy_entry_attempt, status, close_generation, close_owner_token,
      close_reason, created_at, updated_at
    ) VALUES (?, ?, 1, ?, ?, 'registered', NULL, NULL, NULL, 10, 10)
  `).run(instanceId, profileId, requestId, economyEntryAttempt);
  for (const [status, everSeated, at] of [
    ['seat-claimed', 0, 11],
    ['seated', 1, 12],
    ['finished', 1, 13],
...
function prepareWalletEconomyEntry(
  target: DatabaseSync,
  instanceId: string,
  profileId: string,
  entryAttempt: number,
  ...
  target.prepare(`
    INSERT INTO sng_entries (
      ...
      NULL, 0, 0, ?, 10, 10)
  `).run(
    `entry:${instanceId}:${profileId}:${entryAttempt}`,
    instanceId,
    instanceId,
    profileId,
    entryAttempt,
...
function freezeRunningTournament(
  target: DatabaseSync,
  instanceId: string,
): void {
  target.exec(`
    ...
    SET registration_state = 'closing',
        ...
        registration_owner_token = 'test-close-owner'
    WHERE id = '${instanceId}';
    UPDATE tournament_instance
    SET registration_state = 'closed',
        registration_owner_token = NULL,
...
function insertTournamentInstance(
  ...
  const status = options.status ?? 'registering';
  const registrationState = options.registrationState ?? 'open-prestart';
  const started = ['running', 'payout-pending', 'completed'].includes(status);
  const entrantCount = options.entrantCount ?? 2;
  const frozen = started && registrationState === 'closed';
  ...
  `).run(
    ...
    started ? 0 : null,
    started ? entrantCount : null,
    frozen ? entrantCount : null,
    frozen ? 1 : null,
    frozen ? '{}' : null,
    configJson,
    starting ? 'test-start-owner' : null,
    starting ? 100 : null,
    started ? 10 : null,
    terminal ? 10 : null,
  );
