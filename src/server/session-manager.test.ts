...
describe('SessionManager', () => {
  ...
  it('같은 토큰으로 재접속하면 같은 세션(playerId)을 돌려준다', () => {
    ...
    const s1 = sm.resolve('token-a', 'sock-1');
    const s2 = sm.resolve('token-a', 'sock-2');
  ...
  it('detachSocket은 소켓 바인딩만 해제하고 세션은 유지한다', () => {
    const sm = new SessionManager();
    const s = sm.resolve('token-a', 'sock-1');
    s.roomId = 'room-1';
    ...
    expect(sm.getBySocketId('sock-1')).toBeUndefined();
    // 재접속하면 roomId가 남아 있다
    const again = sm.resolve('token-a', 'sock-2');
    expect(again.roomId).toBe('room-1');
  });

  it('이미 새 소켓으로 교체된 구 소켓의 detach는 null (grace 미발동)', () => {
    const sm = new SessionManager();
    sm.resolve('token-a', 'sock-1');
    sm.resolve('token-a', 'sock-2'); // 중복 탭 — 새 소켓 승리
