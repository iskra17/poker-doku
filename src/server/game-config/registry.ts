 ...
 * (캐시 바이인 40~200BB, SnG 바이인/스택/블라인드 스케줄, 투척 쿨다운)은 서버만 바꾸면
 * 화면과 어긋나므로 레지스트리에 넣지 않는다 — 클라 동기화 채널이 생기는 2차에서 확장.
 * 예외적으로 어긋나도 피해가 문구 불일치뿐인 값(타임뱅크 연장)은 warning으로 정직하게 표시.
 */
...
export const GAME_CONFIG_REGISTRY = [
  ...
  },
  {
    ...
    defaultValue: 30_000,
    applyMode: 'immediate',
    warning: '클라이언트 안내 문구 3곳(도움말·액션바·방 만들기)에 "+30초"가 하드코딩되어 있어 변경 시 문구와 어긋남',
  },
  {
