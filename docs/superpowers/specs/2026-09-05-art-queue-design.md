...
- 로컬 OS 파일 잠금(Windows `msvcrt`, 필요 시 POSIX `flock`)으로 같은 작업 루트의 워커가 한 개만 GPU를 사용한다. PID 파일은 표시용이다. 프로세스가 죽으면 OS 잠금은 해제된다.
- ComfyUI에 제출하기 전에 intent와 작업 키를 DB에 커밋한다. 요청의 `extra_data`에 고유 intent ID와 recipe hash를 넣는다. 실제 ComfyUI 소스와 응답을 확인해 queue/history에서 이 키를 회수할 수 있음을 테스트한다.
