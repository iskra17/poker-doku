...
2026-07-25의 `2026-07-25-mtt-freeroll-promotion-fund-design.md`가
MTT `practice`를 실제 지갑 상금의 `freeroll`로 교체하고 공용 운영 원장·상금 에스크로를 추가한다.
wallet 사람 전용 계약은 유지한다.
...
`standard`는 현재 상금표와 동일해 프리롤·지갑 토너먼트의 기본 결과를 바꾸지 않는다. `flat`은 소규모 친선·레크리에이션 대회용, `top-heavy`는 챔피언십형 운영용이다.
...
wallet의 `EconomyRepository.settleMttTournament`는 선택한 프리셋 ID를 입력으로 받고 그 프리셋의 예상 상금과 결과를 대조한다. 멱등 재호출도 프리셋을 포함해 동일 결과인지 검증한다.
프리롤은 후속 설계의 별도 영속 settlement plan과 promotion-funded 정산 경로를 사용한다.

이 상금 프리셋 변경만을 위한 별도 DB 마이그레이션은 만들지 않는다.
후속 `2026-07-25-mtt-freeroll-promotion-fund-design.md`는 예약 MTT용 미배포 v27/v28에
공용 운영 원장·프리롤 상금 에스크로·`payout-pending`을 함께 추가한다.
