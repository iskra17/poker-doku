...
**main ff 병합 완료(태그 `story-act2`), origin 푸시·Fly 배포는 미실행** — 사용자 지시 시 `git push origin main --tags`, `fly deploy --ha=false`(flyctl은 PowerShell로 사용자 홈의 `.fly/bin/flyctl.exe` 실행).
