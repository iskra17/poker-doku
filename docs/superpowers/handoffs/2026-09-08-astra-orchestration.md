...
2026-09-08 후속 사용자 지시: 작은 일에 과도한 비용을 쓰지 않는다. 작은 변경은 구현자 1명과 관련 검사로
마무리하고, 요청하지 않은 기능·설계 문서·다단계 리뷰는 추가하지 않는다. AGENTS의 비용·속도 우선 규칙이
이 문서와 이전 기획의 일반적인 에이전트 배정 절차보다 우선한다.
