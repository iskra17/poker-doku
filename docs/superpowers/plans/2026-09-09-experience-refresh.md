...
## Production deployment — 2026-09-09 KST

- User subsequently requested deployment. Pushed `da7664c` to origin/main and deployed `registry.fly.io/poker-doku:git-da7664c` as Fly **v92**. Release state is `complete`; machine `48ed666a50d2e8` remains the only application machine in `nrt`, started with its HTTP health check passing. Remote Docker production build and TypeScript checks succeeded. The Windows command wrapper returned 1 after Fly printed successful machine checks and DNS verification; deployment success was independently confirmed using the release API, machine image and live HTTP checks.
- Before deployment, the application's native SQLite backup/validation/encryption implementation produced `/data/predeploy-da7664c/poker-doku-2026-09-09.sqlite.enc` (36,687,908 bytes, schema 40) from a read-only database connection. This separate directory preserves the old schema backup when the normal daily backup is replaced. The prior image was `registry.fly.io/poker-doku:deployment-01M1ZDQ16Q2YN37TNK8C7YDE1A` (v91); reverting to it requires a compatible schema-40 database, not an image-only rollback.
- After startup, read-only checks confirmed migrations 41/42, both weekly-dojo tables, 28 supplementary reward catalogue entries totalling 12,400 chips, `PRAGMA quick_check = ok`, and a new encrypted daily backup (36,728,868 bytes). Public `/healthz` returned `{"ok":true}`. A fresh 390px browser context loaded the production onboarding page with HTTP 200, no JavaScript errors, no 5xx responses and no horizontal overflow. No production QA profile or challenge was created. Evidence: `qa-tmp/deploy-da7664c.log`, `deploy-server-da7664c.log`, `deploy-browser-da7664c.log`, `deploy-mobile-da7664c.png`.
- **Existing dependency security follow-up remains:** production dependency audit reports one critical, four high and one moderate affected package. The deployment preserved the reviewed dependency versions; it did not resolve these advisories. Prioritize the [Next.js AVIF image optimization advisory](https://github.com/vercel/next.js/security/advisories/GHSA-2xp9-vwfh-vxw4) (patched in 16.3.3; audit proposes 16.3.4 for the full dependency set) and [Socket.IO parser memory exhaustion](https://github.com/socketio/socket.io/security/advisories/GHSA-2m8v-j782-fhvr) (patched in parser 4.2.7). The audit also lists nanoid, postcss, sharp and baseline-browser-mapping. Application-specific exploitability was not fully assessed; no claim of a clean security audit is made. Full local evidence: `qa-tmp/deploy-audit-da7664c.json`.
