...
# 배포 — ⚠️ 반드시 --ha=false (기본값이 HA로 머신 2대를 만든다)
fly deploy --ha=false
```
...
- 머신은 항상 1대 — 인메모리 단일 인스턴스 구조라 `fly scale count 2` 금지 (수직 확장만).
  최초 `fly deploy`가 기본 HA로 2대를 만들므로 `--ha=false` 필수 (2026-07-14 실제 발생 —
  2대면 프록시가 접속자를 분산시켜 방/좌석 상태가 머신별로 쪼개진다). 이미 2대가 생겼다면
  `fly status`로 확인 후 여분을 `fly machine destroy <id>`. 이후 갱신 배포는 기존 머신 수 유지.
